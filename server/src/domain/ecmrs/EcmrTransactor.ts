...
export class EcmrTransactor implements TransactionCreator {
  ...
  public async getEcmrsByVin(transactionHandler: TransactionHandler, identity: Identity, connectionProfile: string, vin: string): Promise<any> {
    let vehicle       = await transactionHandler.executeQuery(identity, connectionProfile, QueryReturnType.Single, Queries.GetVehicleByVin, {vin: vin});
    let result: any[] = [];
    if (!vehicle) {
      return result;
    } else {
      for (let ecmr of vehicle.ecmrs) {
        ...
        await transactionHandler.executeQuery(identity, connectionProfile, QueryReturnType.Single, Queries.GetEcmrById, {ecmrID: ecmrID}).then((ecmr) => {
  ...
  public async getEcmrsByPlateNumber(transactionHandler: TransactionHandler, identity: Identity, connectionProfile: string, plateNumber: string): Promise<any> {
    // get vehicle by vin
    const vehicle     = await transactionHandler.executeQuery(identity, connectionProfile, QueryReturnType.Single, Queries.GetVehicleByPlateNumber, {plateNumber: plateNumber});
    let result: any[] = [];
    if (!vehicle) {
      return result;
    } else {
