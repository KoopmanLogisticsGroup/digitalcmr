...
NUM_PENDING=$(kubectl get deployments | grep kpm-all | grep -v composer | wc -l | awk '{print $1}')
...
NUM_PENDING=$(kubectl get svc | grep kpm-all | grep -v composer | wc -l | awk '{print $1}')
while [ "${NUM_PENDING}" != "0" ]; do
