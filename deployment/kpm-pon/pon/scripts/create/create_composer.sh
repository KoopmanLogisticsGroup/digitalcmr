...
kubectl cp $PON_PATH/$PON_USERS_PARTIAL_PATH/ $POD_NAME:$CONTAINER_BASE_PATH/

while [ "$(kubectl get pods -a | grep composer-identity-import-pon | awk '{print $3}')" != "Completed" ]; do
    echo "Waiting for composer-identity-import container to be Completed"
    sleep 1;
done

if [ "$(kubectl get pods -a | grep composer-identity-import-pon | awk '{print $3}')" == "Completed" ]; then
...
while [ "$(kubectl get svc | grep composer-identity-import-pon | wc -l | awk '{print $1}')" != "0" ]; do
...
#
#if [ "$(kubectl get svc | grep composer-playground-pon | wc -l | awk '{print $1}')" == "0" ]; then
...
#
#NUMPENDING=$(kubectl get deployments | grep composer-playground-pon | awk '{print $5}' | grep 0 | wc -l | awk '{print $1}')
